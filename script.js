...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Inicializa o turno ativo com base no HTML (se houver um botão com .active)
    ...
    function updateDateTime() {
        ...
        if (currentDateSpan) currentDateSpan.textContent = now.toLocaleDateString('pt-BR', dateOptions);
        if (currentTimeSpan) currentTimeSpan.textContent = now.toLocaleTimeString('pt-BR', timeOptions);
    }
    ...
    function entrarModoContagem() {
        ...
        window.dispatchEvent(new Event('resize')); // Força recalculo de layout
    ...
    function sairModoContagem() {
        ...
        if (btnAtivarModoContagem) btnAtivarModoContagem.style.display = 'inline-block'; // Mostra o botão de ativar novamente
        window.dispatchEvent(new Event('resize')); // Força recalculo de layout
    ...
    turnosButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            selectedTurno = button.dataset.turno;
            carregarCardapioAluno(); // Recarrega/filtra o cardápio ao mudar o turno
        });
    });
    ...
    function loadMerendaData() {
        ...
        } catch (e) { console.error('Erro ao carregar dados do localStorage para merenda:', e); }
    ...
    function saveMerendaData(contagemMerenda, numConfirmacoes) {
        ...
        } catch (e) { console.error('Erro ao salvar dados de merenda no localStorage:', e); }
    }
    ...
    function getCardapioFromLocalStorage() {
        ...
        } catch (e) { console.error('Erro ao carregar cardápio do localStorage:', e); return []; }
    }
    ...
    function carregarCardapioAluno() {
        ...
        const cardapioFiltrado = cardapioCompleto.filter(item => item.periodo === selectedTurno || item.periodo === 'Todos');

        if (cardapioFiltrado.length === 0) {
            if (cardapioItensDiv) cardapioItensDiv.innerHTML = `<p class="info-message">Nenhum item de merenda disponível para o turno da ${selectedTurno.toLowerCase()} hoje.</p>`;
            if (confirmarEscolhaBtn) {
                confirmarEscolhaBtn.disabled = true;
                confirmarEscolhaBtn.classList.add('disabled');
            }
    ...
    if (confirmarEscolhaBtn) {
        confirmarEscolhaBtn.addEventListener('click', () => {
            ...
            const cardapioCompleto = getCardapioFromLocalStorage();
            const cardapioVisivel = cardapioCompleto.filter(item => item.periodo === selectedTurno || item.periodo === 'Todos');
            let totalItensEscolhidos = 0;

            cardapioVisivel.forEach(item => {
                const cardElement = document.querySelector(`.item-merenda-chic[data-id="${item.id}"]`);
                ...
                if (item.tipo === 'unidade') {
                    ...
                    if (quantitySpan) {
                        ...
                        if (quantity > 0) {
                            contagemMerenda[item.id] = (contagemMerenda[item.id] || 0) + quantity;
            ...
            if (totalItensEscolhidos > 0) {
                ...
                // Apenas reseta as seleções visuais para a próxima escolha no Modo Contagem
                document.querySelectorAll('.item-merenda-chic.selected').forEach(card => card.classList.remove('selected'));
                document.querySelectorAll('.item-quantity').forEach(span => span.textContent = '0');
                
                // NÃO SAI MAIS DO MODO CONTAGEM AUTOMATICAMENTE AO CONFIRMAR
                // if (document.body.classList.contains('modo-contagem-ativo')) {
                //    // A linha que chamava sairModoContagem() foi removida daqui.
                //    // Se precisar de um refresh completo dos cards no modo contagem, pode chamar carregarCardapioAluno()
    ...
    carregarCardapioAluno(); // Carga inicial
